...
Hobbin.prototype = {
	...
	step: function() { // Taken from the java-port of Digger @ digger.org
	  ...
	  if (this.xr==0 && this.yr==0) {
		...
		if (Math.abs(digger.y-this.y)>Math.abs(digger.x-this.x)) {
		  if (digger.y<this.y) {
			mdirp1=2; mdirp4=6;
		  }	else {
			mdirp1=6; mdirp4=2;
	  ...
	  /* Hobbins digger */
	  if (!this.nob) {
		digger.Drawing.eatfield(this.x,this.y,this.dir);
	  }
	  ...
	  switch (this.dir) {
		case 0:
		  if (!this.nob)
			digger.Drawing.drawrightblob(this.x,this.y);
		  this.x+=4;
		  break;
		case 4:
		  if (!this.nob)
			digger.Drawing.drawleftblob(this.x,this.y);
		  this.x-=4;
		  break;
		case 2:
		  if (!this.nob)
			digger.Drawing.drawtopblob(this.x,this.y);
		  this.y-=3;
		  break;
		case 6:
		  if (!this.nob)
			digger.Drawing.drawbottomblob(this.x,this.y);
		  this.y+=3;
		  break;
	  }
	  ...
	  if (!this.nob) {
			digger.hitemerald((this.x-12)/20,(this.y-18)/18, (this.x-12)%20,(this.y-18)%18, this.dir);
	  }
	  ...
	  push=true;
	  clbits=digger.Drawing.drawmon(mon,this.nob,this.hdir,this.x, this.y);
	  ...
	  if (!push) {
		this.x=this.x;
		this.y=this.y;
		digger.Drawing.drawmon(mon,this.nob,this.hdir,this.x,this.y);
		digger.Main.incpenalty();
